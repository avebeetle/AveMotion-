...
Push-Location $repo
try {
    Write-Host "[AveMotion Part 22] Configure: $Preset"
    cmake --preset $Preset
    if ($LASTEXITCODE -ne 0) { throw "CMake configure failed." }

    Write-Host "[AveMotion Part 22] Build: $Preset"
    cmake --build --preset $Preset --parallel
    if ($LASTEXITCODE -ne 0) { throw "CMake build failed." }

    Write-Host "[AveMotion Part 22] Hardened TGS + central player + Win32 lifecycle validation"
    ctest --preset $Preset `
        -R "avemotion\.(cmake\.(win32_manifest_wiring|player_wiring|tgs_wiring)|formats\.tgs|runtime\.tgs|player\.scheduler|win32\.preview\.selftest|direct2d\.(header|contract|smoke))" `
        -V --output-on-failure
    if ($LASTEXITCODE -ne 0) { throw "TGS / central player / Win32 lifecycle validation failed." }

    if (-not $SkipCapture) {
        Write-Host "[AveMotion Part 22] Native Direct2D capture corpus"
        ctest --preset $Preset `
    ...
    if (-not $SkipFullSuite) {
        Write-Host "[AveMotion Part 22] Remaining Telegram + Direct2D suite"
        $excludedTests = @(
            '^avemotion\.cmake\.(win32_manifest_wiring|player_wiring|tgs_wiring)$',
    ...
    if (-not (Select-String -LiteralPath $previewReport -Pattern '^asset=.*\.tgs$')) {
        throw "Win32 preview self-test did not load the Part 22 TGS fixture."
    }
    ...
    Write-Host "[AveMotion Part 22] PASS"
    ...
    if (-not $NoLaunch) {
        ...
        Write-Host "[AveMotion Part 22] Launching interactive TGS preview through the centralized Player."
