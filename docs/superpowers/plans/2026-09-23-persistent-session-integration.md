...
- [ ] Make the validator behavior test self-contained when run by itself: pass the built corpus executable with `$<TARGET_FILE:avemotion_corpus_lab>` and source corpus path, generate real reports in its own temporary directory, then copy/mutate those rows. Do not depend on archived out/benchmarks reports, hard-coded preset executable paths or a prior unrequested CTest run. Synthetic fresh-profile count variants may derive from real candidate rows; Task4 validates actual historical binaries separately.
