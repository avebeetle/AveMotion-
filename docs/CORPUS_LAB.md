...
eagerly creates one recording Scene session per Instance at setup and reuses
it for later exact samples, which create zero new sessions. A Telegram outlier
with a wrapped frame count greater than `LONG_MAX` uses that same session for
mapping. Each successful model preparation uses one temporary recording
session for the complete ascending scan; sample count equals manifest frames.
CPU rendering remains a separate lazy ordinary session.

The output validator defaults to `--lifetime-profile persistent`: setup has
metadata=1, scene=1, model=1, CPU=0; first/steady sessions are all zero.
First/steady scene samples remain 1/the measured CLI sample count.
Historical Original reports require `--lifetime-profile fresh
--expected-setup-scene-sessions 1`; historical Part25B reports require
`--lifetime-profile fresh` (setup scene default 0). Both fresh profiles require
one model session per manifest frame and one scene session per scene sample.
The optional setup override does not relax any other phase or role count.
These creation counters do not prove allocation-free evaluation.
